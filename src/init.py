...
dataset_url = "http://mallba3.lcc.uma.es/jamorell/datasets/ham10000/gray/normal/base_dir/train_dir.zip"
dataset_path = "./" + dataset_name + "/train_dir/" 
runs_url = "./output/runs" #"./output_transforms/runs"
models_url = "./output/models" #"./output_transforms/models"
images_url = "./output/images" #"./output_transforms/images"
...
num_epochs = 40000 #5000 #20
n_critic = 50
display_step = 2000 # 10
loaded = load_checkpoint(generator, discriminator, g_optimizer, d_optimizer, models_url) #,epoch
epoch = loaded[0]
saved_real = False
step = 0
saved_step = loaded[1]
save_model_each_n_epochs = 20
fretchet_dist = 99999999
######################LOOP
...
for epoch in range(epoch, num_epochs):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        if (is_oversampling):
          labels = torch.tensor(np.array(labels, 'int32'))
        ...
        if step % display_step == 0:
          ...
          try:
            ...
            fretchet_dist = 99999999
          #fretchet_dist = calculate_fretchet(real_images, fake_images, model_fid, batch_size, image_size, device) 
          print("step = " + str(step) + "type(fretchet_dist) = " + str(type(fretchet_dist)))
          #print("fretchet_dist = " + str(fretchet_dist))
          mywritter.add_scalar('fretchet_dist', fretchet_dist)   
        ###############
        print("epoch = " + str(epoch) + " step = " + str(step) + " fid = " + str(fretchet_dist) + " g_loss = " + str(g_loss) + " d_loss = " + str(d_loss))
        ...
        if step % display_step == 0:
          ...
          save_image(grid, images_url + '/_' + str(epoch) + '_' + str(step) + '_' + str(fretchet_dist) + '.png')
